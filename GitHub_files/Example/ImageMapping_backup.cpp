...
private:
	...
	virtual bool Iterate(const Math::Vector& params, Math::Vector& results)
	{
		...
		translation.at<float>(0, 0) = (translation_start.at<float>(0, 0) + 1000000 * (float) params[0]);
		translation.at<float>(1, 0) = (translation_start.at<float>(1, 0) + 1000000 * (float) params[1]);
		translation.at<float>(2, 0) = (translation_start.at<float>(2, 0) + 1000000 * (float) params[2]);
