...
struct ImageMapping : public ogx::Plugin::EasyMethod, OptimizeRotation, OptimizeTranslation
{
	...
	void silhouetteFromImage(cv::Mat &img_in, cv::Mat &img_out)
	{
		...
		int prev, next;
		int tol = img_in.cols*img_in.rows*0.001;
	...
	virtual void Run(Context& context)
	{
		...
		for (int i = 16; i > 0; i--)
		{
			...
			optimRot.Optimize();
			camera.calculateRvec(optimRot.view);
			rotVec = camera.rvec.clone();
			camera.calculateTvec(optimRot.view);
			transVec = camera.tvec.clone();
