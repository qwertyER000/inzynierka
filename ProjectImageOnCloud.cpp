		...
		else
		{
			for (auto& ipt : pointsRGB)
			{
				...
				else
				{
					...
					ipt.z() = 0.6* current_color[0] + 0.4 * imgCam.at<cv::Vec3b>(tmp.y, tmp.x)[0];
					ipt.y() = 0.6* current_color[1] + 0.4 * imgCam.at<cv::Vec3b>(tmp.y, tmp.x)[0];
					ipt.x() = 0.6* current_color[2] + 0.4*  imgCam.at<cv::Vec3b>(tmp.y, tmp.x)[0];
